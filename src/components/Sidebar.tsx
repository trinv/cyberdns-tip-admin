...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems = [
    {
      group: 'TỔNG QUAN',
      items: [{ id: 'dashboard', label: 'SOC Dashboard', icon: LayoutDashboard }],
    },
    {
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`h-16 px-4 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} border-b border-slate-100 dark:border-slate-800/80`}
        >
          <div
        ...
        <div
          className={`p-3 border-b border-slate-100 dark:border-slate-800/80 ${isCollapsed ? 'flex justify-center' : ''}`}
        >
          {isCollapsed ? (
            <div
              className="relative cursor-pointer group p-1"
              title={currentUser ? `${currentUser.email} — ${userRole}` : 'Chưa đăng nhập'}
            >
              ...
              {currentUser && (
                <span className="absolute bottom-0 right-0 w-3 h-3 rounded-full bg-green-500 ring-2 ring-white dark:ring-slate-900"></span>
              )}
            </div>
          ) : (
              ...
              <div className="flex items-center space-x-2.5 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate flex items-center space-x-1.5 mt-0.5">
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${currentUser ? 'bg-green-500' : 'bg-slate-400'}`}
                    ></span>
                  ...
                  {sec.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center space-x-2.5 min-w-0">
                          <Icon
                            className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-primary' : 'text-slate-400 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-200'}`}
                          />
